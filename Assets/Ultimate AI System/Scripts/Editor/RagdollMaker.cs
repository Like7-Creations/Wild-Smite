...
{
    public class RagdollMaker : ScriptableWizard
    {
        public Transform ai;
        [Space(20f)]
        ...
        string CheckConsistency()
        {
            ...
            if (ai == null)
            {
                return "Make sure to reference your AI object in the \"Ai\" field.\n";
            }
        ...
        [MenuItem("GameObject/3D Object/Ultimate AI Ragdoll...", false, 2000)]
        static void CreateWizard()
        {
            ScriptableWizard.DisplayWizard<RagdollMaker>("Create Ragdoll");
        }
        ...
        void OnWizardUpdate()
        {
            if (ai != null && !madeCheck)
            {
                Animator anim = ai.GetComponent<Animator>();

                if (ai.GetComponent<Animator>())
                {
                    pelvis = anim.GetBoneTransform(HumanBodyBones.Hips);
                    leftHips = anim.GetBoneTransform(HumanBodyBones.LeftUpperLeg);
                    leftKnee = anim.GetBoneTransform(HumanBodyBones.LeftLowerLeg);
                    leftFoot = anim.GetBoneTransform(HumanBodyBones.LeftFoot);
                    rightHips = anim.GetBoneTransform(HumanBodyBones.RightUpperLeg);
                    ...
                    middleSpine = anim.GetBoneTransform(HumanBodyBones.Spine);
        ...
        void OnWizardCreate()
        {
            ...
            Debug.Log($"Successfully created a ragdoll for the following object: {ai.name}.");

            ai.GetComponent<UltimateAI>().ragdollCreated = true;
        }
        ...
        void BuildBodies()
        {
            foreach (BoneInfo bone in bones)
            {
                ...
                bone.anchor.GetComponent<Rigidbody>().collisionDetectionMode = CollisionDetectionMode.Continuous;
            }
        }
